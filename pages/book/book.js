...
Page({
  ...
  data: {
    objData:[],
    showData:[],
    nav:[{id:1,name:"全部"},{id:2,name:"买书"},{id:3,name:"卖书"}],
    currentIndex:1,
    isShow:false,
    length:0,                 //话题数
    chooseData:[],
  ...
  drop:function(e){
   ...
   // console.log(index);
    //找到点击跳转文章的id
    var id = this.data.chooseData[index].school_book_id;
    ...
    wx.navigateTo({
      url: '/pages/book/desbook/desbook',
      success:(res) =>{
        res.eventChannel.emit('bookDetail',obj);
      }
    })
  },
  ...
  chooseData:function(index){
    ...
    if(index == 1){
      ...
      for(var i = 0; i<length;i++){
        if(chooseData[i].school_book_type == '买书'){
      ...
      data = [];
      for(var i = 0; i<length;i++){
        if(chooseData[i].school_book_type == '卖书'){
          data.push(chooseData[i]);
        }
      }
    }
  ...
  changeimg:function(e){
    ...
    for(var i=0;i<this.data.chooseData.length;i++){
      ...
      if (this.data.chooseData[i].school_book_id == setIndex){
    ...
    if (isShowHeart == false || isShowHeart == null){
      show[heartdata.school_book_id] = true;
      action = 'success';
      wx.showToast({
        title: "点赞成功",
        icon: "none",
      ...
      show[heartdata.school_book_id] = false;
      action = 'cancel';
      wx.showToast({
        title: "取消点赞",
        icon: "none",
    ...
    this.setData({
      ...
      wx.request({
        ...
        data:{
          "action":action,
          "school_type":2,
          "school_id":heartdata.school_book_id,
        },
  ...
  //  console.log(trueData);
    ...
    for(var i=0;i<trueData.length;i++){
      /**处理文章的时间 */
      var time = util.formatTime(new Date(trueData[i].school_book_time));

      var str = trueData[i].school_book_path;            //获取数组中每个对象中的全部图片
      var s_str = str.split(",");                         //将图片字符串按，分隔开
      // console.log(s_str);
      if(s_str.length>0){                                //当有图片的时候
        showData[i] = trueData[i];                       //将数据赋值给showData中
        showData[i].school_book_path = s_str;           //将数组型的图片地址赋值到showData中
      }else{
        showData[i] = trueData[i];                       //没有图片时，将将原数据赋值回去
      }
      showData[i].school_book_time = time;            //转换时间形式
      /*将type显示成中文 */
      var type = trueData[i].school_book_type
      if(type == 0){
        showData[i].school_book_type = '买书'
      }else if(type == 1){
        showData[i].school_book_type = '卖书'
      }
      }
      that.setData({
        showData: showData,
      ...
      for (var i = 0; i < that.data.chooseData.length; i++) {
        for(var j=0;j<that.data.like_essay.length;j++){
          if(that.data.chooseData[i].school_book_id == that.data.like_essay[j].school_essay_id){
         /*   console.log(that.data.showData[i].school_experience_id);
            console.log(that.data.showData[i]);*/
          //  console.log(that.data.like_essay);
            isShow[that.data.chooseData[i].school_book_id] = true
          }
        }
      }
      that.setData({
 ...
 formSubmit:function(e){
  ...
  onLoad: function (options) {
    wx.request({
      url: 'https://www.younghao.top/data/data?key=2',
      method:'GET',
      header:  {"auth":wx.getStorageSync("login_key")},
      success:(res) =>{
        this.setData({
