...
Page({
  ...
  data: {
    ...
    length:0,                 //话题数
    showData:[],
    nav:[{id:0,name:"全部"},{id:1,name:"学习"},{id:2,name:"生活"},{id:3,name:"工作"}],
    currentIndex:0,
    chooseData:[],
    like_essay:[],
  ...
  chooseData:function(index){
    ...
    if(index == 0){
      ...
      for(var i = 0; i<length;i++){
        if(chooseData[i].school_experience_type == '学习'){
      ...
      data = [];
      for(var i = 0; i<length;i++){
        if(chooseData[i].school_experience_type == '生活'){
          data.push(chooseData[i]);
        }
      }
    }else if(index == 3){
      data = [];
      for(var i = 0; i<length;i++){
        if(chooseData[i].school_experience_type == '工作'){
          data.push(chooseData[i]);
        }
      }
    }
  ...
  changeimg:function(e){
    ...
    for(var i=0;i<this.data.chooseData.length;i++){
      if (this.data.chooseData[i].school_experience_id == setIndex){
    ...
    if (isShowHeart == false || isShowHeart == null){
      show[heartdata.school_experience_id] = true;
      action = 'success';
      wx.showToast({
        title: "点赞成功",
        icon: "none",
      ...
      show[heartdata.school_experience_id] = false;
      action = 'cancel';
      wx.showToast({
        title: "取消点赞",
        icon: "none",
    ...
    this.setData({
      ...
      wx.request({
        ...
        data:{
          "action":action,
          "school_type":1,
          "school_id":heartdata.school_experience_id,
        },
  ...
  drop:function(e){
    ...
    var id = this.data.chooseData[index].school_experience_id;
    ...
    wx.navigateTo({
      url: '/pages/experience/desexperience/deseperience',
      success:(res) =>{
        res.eventChannel.emit('experienceDetail',obj);
      }
    })
  },
  /*改变图片的地址（从字符串变为数组）/ 改变时间的显示格式  / 点赞的初始化*/
  changeImage:function(){
   ...
   // console.log(trueData);
    ...
    for(var i=0;i<trueData.length;i++){
      /**处理文章的时间 */
      var time = util.formatTime(new Date(trueData[i].school_experience_time));

      var str = trueData[i].school_experience_path;            //获取数组中每个对象中的全部图片
      var s_str = str.split(",");                         //将图片字符串按，分隔开
      if(s_str.length>0){                                //当有图片的时候
        showData[i] = trueData[i];                       //将数据赋值给showData中
        showData[i].school_experience_path = s_str;//将数组型的图片地址赋值到showData中
      }else{
        showData[i] = trueData[i];                     //没有图片时，将将原数据赋值回去
      }
      showData[i].school_experience_time = time;            //转换时间形式
      /*将type显示成中文 */
      var type = trueData[i].school_experience_type
      if(type == 0){
        showData[i].school_experience_type = '学习'
      }else if(type == 1){
        showData[i].school_experience_type = '生活'
      }else if(type == 2){
        showData[i].school_experience_type = '工作'
      }
      }
      that.setData({
        showData: showData,
        chooseData:showData         //方便后面切换模块
      })
      /**初始化点赞的状态组 */
      var isShow = {};
      for (var i = 0; i < that.data.chooseData.length; i++) {
        for(var j=0;j<that.data.like_essay.length;j++){
          if(that.data.chooseData[i].school_experience_id == that.data.like_essay[j].school_essay_id){
            isShow[that.data.chooseData[i].school_experience_id] = true
          }
        }
      }
      that.setData({
  ...
  onLoad: function (options) {
    wx.request({
      url: 'https://www.younghao.top/data/data?key=1',
      method:'GET',
      header:  {"auth":wx.getStorageSync("login_key")},
      success:(res) =>{
        console.log(res);
        this.setData({
          objData: res.data.promise,          //所有内容
          length:res.data.promise.length,     //模块长度
          like_essay:res.data.like_essay,      //点赞信息
          responseLength:res.data.response_length  //评论数
        },data=>{
