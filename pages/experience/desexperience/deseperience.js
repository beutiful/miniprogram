// pages/experience/desexperience/deseperience.js
...
Page({
    ...
    this.setData({
      ...
      wx.request({
        ...
        data:{
          "action":action,
          "school_type":1,
          "school_id":this.data.objData.school_experience_id,
        },
  ...
  previewImg: function(e){
    ...
    // 设置预览图片路径
    wx.previewImage({
      current: this.data.objData.school_experience_path[index],
      urls: this.data.objData.school_experience_path  
     })
  },
  /**可以直接在setData中设置data中不存在的值 */
  /*加遮罩 */
  ...
  no:function(e){
   ...
   /*处理文章中的空格 */
  lineFeed:function(){
  //  console.log(this.data.objData.school_experience_content);
    var content = (this.data.objData.school_experience_content).replace(/\/n/g,"\n");
    this.setData({
      content:content
    })
  },
  ...
  countPeople:function(){
      wx.request({
        ...
        data:{
          ...
          "school_experience_id":this.data.objData.school_experience_id,
          "school_experience_people":this.data.objData.school_experience_people
        },
        header: {"auth":wx.getStorageSync("login_key")},
        success: (res) => {
  ...
  formSubmit:function(e){
    ...
    wx.request({
      ...
      data:{
        "content":content,
        "school_type":1,
        "school_id":that.data.objData.school_experience_id
      },
      header: {"auth":wx.getStorageSync("login_key")},
      success: (res) => {
        if (res.data.status=='200'){
  ...
  onLoad: function (options) {
    ...
    eventChannel.on("experienceDetail", data =>{                //接收传递的数据
      that.setData({
        objData:data.objData,
        show:data.show,
        resLength:data.responseLength
      },data =>{
        console.log(this.data.objData);
        that.commentData();
      }); 
