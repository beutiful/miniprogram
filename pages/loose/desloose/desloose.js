// pages/loose/desloose/desloose.js
...
Page({
      ...
      this.setData({
        ...
        wx.request({
          ...
          data:{
            "action":action,
            "school_type":4,
            "school_id":this.data.objData.school_found_id,
          },
  ...
  previewImg: function(e){
    ...
    // 设置预览图片路径
    wx.previewImage({
      current: this.data.objData.school_found_path[index],
      urls: this.data.objData.school_found_path  
     })
  },
  /*加遮罩 */
  ...
  no:function(e){
   ...
   /*处理文章中的空格 */
  lineFeed:function(){
    var content = (this.data.objData.school_found_content).replace(/\/n/g,"\n");
    this.setData({
      content:content
    })
  },
  ...
  countPeople:function(){
      wx.request({
        ...
        data:{
          "table":"school_found",
          "school_found_id":this.data.objData.school_found_id,
          "school_found_people":this.data.objData.school_found_people
        },
        header: {"auth":wx.getStorageSync("login_key")},
        success: (res) => {
...
formSubmit:function(e){
  ...
  wx.request({
    ...
    data:{
      "content":content,
      "school_type":4,
      "school_id":that.data.objData.school_found_id
    },
    header: {"auth":wx.getStorageSync("login_key")},
    success: (res) => {
      if (res.data.status=='200'){
...
isShowResponse:function(){
  ...
  onLoad: function (options) {
    ...
    eventChannel.on("looseDetail", data =>{                 //接收传递的数据
      that.setData({
