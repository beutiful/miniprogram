...
Page({
      ...
      this.setData({
        ...
        wx.request({
          ...
          data:{
            "action":action,
            "school_type":5,
            "school_id":this.data.objData.school_employ_id,
          },
  ...
  previewImg: function(e){
    ...
    // 设置预览图片路径
    wx.previewImage({
      current: this.data.objData.school_employ_path[index],
      urls: this.data.objData.school_emlpoy_path  
     })
  },
  /*加遮罩 */
  ...
  no:function(e){
   ...
   /*处理文章中的空格 */
  lineFeed:function(){
    var content = (this.data.objData.school_employ_content).replace(/\/n/g,"\n");
    this.setData({
      content:content
    })
  },
  ...
  countPeople:function(){
    wx.request({
      ...
      data:{
        "table":"school_employ",
        "school_employ_id":this.data.objData.school_employ_id,
        "school_employ_people":this.data.objData.school_employ_people
      },
      header: {"auth":wx.getStorageSync("login_key")},
      success: (res) => {
...
formSubmit:function(e){
  ...
  wx.request({
    ...
    data:{
      "content":content,
      "school_type":5,
      "school_id":that.data.objData.school_employ_id
    },
    header: {"auth":wx.getStorageSync("login_key")},
    success: (res) => {
      if (res.data.status=='200'){
...
isShowResponse:function(){
  ...
  onLoad: function (options) {
    ...
    eventChannel.on("employDetail", data =>{                 //接收传递的数据
      that.setData({
